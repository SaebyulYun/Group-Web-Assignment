...
if ($postId) {
    ...
    if ($filterType == 'content') {
        $query = "SELECT * FROM blog_posts WHERE content REGEXP CONCAT('[[:<:]]', ?, '[[:>:]]')";
    }
    elseif ($filterType == 'category') {
        $query = "SELECT * FROM blog_posts WHERE category REGEXP CONCAT('[[:<:]]', ?, '[[:>:]]')";
    }
    elseif ($filterType == 'author') {
        $query = "SELECT * FROM blog_posts WHERE author REGEXP CONCAT('[[:<:]]', ?, '[[:>:]]')";
    } 
    else {
        $query = "SELECT * FROM blog_posts WHERE title REGEXP CONCAT('[[:<:]]', ?, '[[:>:]]')";
    }
    $stmt = $conn->prepare($query);
